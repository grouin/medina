...
# passées en argument de l'option -c)

# Usage : perl post_pseudonymization.pl -r répertoire/ -e extension [-c Personne,Telephone]
...
foreach my $entree (@rep) {
  ...
  while (my $ligne=<E>) {
    ...
    # "le Organisation")
    foreach my $categorie (@categories) { $ligne=~s/<$categorie>[^<]+<\/$categorie>/<$categorie \/>/g; }
