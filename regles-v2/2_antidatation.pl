...
# Gestion des options
&getopts("r:n:e:m");
#if (!$opt_r) { die "Usage :\tperl 2_antidatation.pl -r <répertoire> [-e <extension>] [-n <nombre de jours>]\n"; }
...
my $anneeDefaut=2014;
my $soustrait;
($opt_e) ? ($extension=$opt_e) : ($extension="med");
...
if ($opt_r) {
	...
	$sortie=substr($fichier,0,length($fichier)-3)."dat";
	$log=$sortie.".log";
	if (!-e $sortie) {
...
sub traiteFichier() {
	...
	if (length($date)>=44) {
	    $nouvelle=$ligne;
	    if ($opt_m) {
		$ligne=~s/<date>\Q$date\E<\/date>/DATE/; 
		$nouvelle=~s/<date>\Q$date\E<\/date>/<date>DATE<\/date>/; 
	    }
	    else { $ligne=~s/<date>\Q$date\E<\/date>/DATE/; $nouvelle=$ligne; }
	    next; 
	}
	...
	if ($ok==1) {
		...
		$ligne=~s/<date>$date<\/date>/$fin{$date}/g;
		if ($opt_m) { $nouvelle=~s/<date>$date<\/date>/<date>$fin{$date}<\/date>/g; }
		else { $nouvelle=~s/<date>$date<\/date>/$fin{$date}/g; }
	    }
		...
		$ligne=~s/<date>$date<\/date>/DATE/g;
		if ($opt_m) { $nouvelle=~s/<date>$date<\/date>/<date>DATE<\/date>/g; }
		else { $nouvelle=~s/<date>$date<\/date>/DATE/g; }
	    }
...
sub datation() {
	...
	# Sinon, on considère que le deuxième élément est un mois
	    ...
	    $corr{$date}="$cols[0] $cols[1] $anneeDefaut"; 
	}
	...
	$corr{$date}="$cols[0] $cols[3] $anneeDefaut au $cols[2] $cols[3] $anneeDefaut";
    }

    # Si 5 éléments, intervalle de dates : 2 au 4 septembre 2006
    elsif ($#cols==4) {
	$corr{$date}="$cols[0] $cols[3] $cols[4] au $cols[2] $cols[3] $cols[4]";
    }

    # Si 6 éléments, énumération de dates : 12 13 14 et 15 juin
    elsif ($#cols==5) {
	$corr{$date}="$cols[0] $cols[4] $cols[5] au $cols[3] $cols[4] $cols[5]";
    }
